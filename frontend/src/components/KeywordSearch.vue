  ...
  methods:{
    searchCity(key){
      if(key==null)
      {
        ...
        axios.get("http://localhost:8081/getRoom",{params:{city:key}}).then((Response)=>{
        this.$bus.$emit('getData',Response.data)
        });
      }
    },
    searchMoney(key1,key2){
       if(key1==null)
      {
        ...
        axios.get("http://localhost:8081/getRoom",{params:{lowmoney:key1,highmoney:key2}}).then((Response)=>{
        this.$bus.$emit('getData',Response.data)
        });
      }
    },
    searchType(key){
       if(key==null)
      {
        ...
        axios.get("http://localhost:8081/getRoom",{params:{type:key}}).then((Response)=>{
        this.$bus.$emit('getData',Response.data)
        });
      }
    }
